...
def transpose_for_scores(input_tensor):
    output_tensor = torch.reshape(
        input_tensor, (batch_size, seq_length, num_attention_heads, attention_head_size))
...
def create_attention_mask(from_seq_length, to_mask):
    ...
    to_mask = torch.reshape(
        to_mask, (batch_size, 1, to_seq_length))  # [B,T] ->[B,1,T]
...
def attention_layer(from_tensor, to_tensor, attention_mask=None):
    ...
    attention_scores = torch.matmul(q, torch.transpose(
        k, 2, 3))  # [B,N,F,H]* [B,N,H,T] -> [B,N,F,T]
    d_sqrt = 1/math.sqrt(float(width))
    if attention_mask is not None:
        # [B, F, T] -> [B, 1, F, T] 扩增了纬度，这里边的元素只有0和1两种，mask的标识是0，需要注意力的标识是1。
    ...
    v = torch.reshape(to_tensor, (batch_size, seq_length,
                      num_attention_heads, attention_head_size))  # [B,T,N,H]
    v = torch.transpose(v, 2, 1)  # [B,N,T,H]

    y = torch.matmul(attention_porbs, v)  # [B,N,F,T] * [B,N,T,H] -> [B,N,F,H]
    y = torch.transpose(y, 2, 1)  # [B,F,N,H]
    y = torch.reshape(to_tensor, (batch_size, seq_length,
                      num_attention_heads*attention_head_size))  # [B,F,N*H]
    return y
